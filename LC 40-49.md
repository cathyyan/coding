...
//           OR all m[k][j - 1] for k in [0, i], if p[j] == '*'
...
class Solution {
 public:
  bool isMatch(const std::string& s, const std::string& p) {
    ...
    std::cout << s.length() << " " << p.length() << std::endl;
    
    std::vector<std::vector<int>> m(s.length(),
                                    std::vector<int>(p.length(), 0));

    // Compute m[i][0] for all i.
    for (std::size_t i = 0; i < s.length(); ++i) {
      m[i][0] = p[0] == '*' || (i == 0 && (p[0] == '?' || p[0] == s[i]));
      std::cout << i << " " << 0 << " --> " << m[i][0] << std::endl;
    }
